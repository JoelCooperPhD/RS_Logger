...
from RSLogger.devices.wdrt.UserInterface import WDRT_UIPlotter
...
class WDRTMainWindow:
    ...
    def _tab_add_battery_bar(self):
        lf = LabelFrame(self._tab_f['frame'], text="Battery Charge")
        lf.grid(row=0, column=1, columnspan=2, sticky="NEWS")
        lf.grid_columnconfigure(0, weight=1)

        self._tab_f['bat_f'] = Frame(lf)
        self._tab_f['bat_f'].grid(row=0, column=0, sticky="NEWS")

        for i in range(10):
            self._tab_f['bat_f'].grid_columnconfigure(i, weight=1)
            self._tab_f[f'b_{i}'] = tkLabel(self._tab_f['bat_f'], borderwidth=0, bg='white', relief="sunken")
            self._tab_f[f'b_{i}'].grid(row=0, column=i, sticky="NEWS", pady=2, padx=1)
    ...
    def battery_bar(self, percent=0):
        p = percent // 10
        for i in range(10):
            color = 'white'
            if p >= i + 1:
            ...
            self._tab_f['bat_f'].grid_columnconfigure(i, weight=1)
            tkLabel(self._tab_f['bat_f'], borderwidth=0, bg=color, relief="sunken").\
                grid(row=0, column=i, sticky="NEWS", pady=2, padx=1)
