...
class WirelessVOG:
    def __init__(self, serial):
        ...
        self.exp_cycle = experiments.Cycle(self.cfg.config)
        self.exp_direct = experiments.Direct()
    ...
    def handle_do(self, key, val):
        if key == 'lens': self.update_lens_states(key, val)
        elif key == 'start': self.start_experiment(val)
        elif key == 'stop' : self.stop_experiment(val)
    ...
    def handle_get(self, key, val):
        if   key == 'cfg': self.get_config(val)
    ...
    def start_experiment(self, val):
        if   val == "peek": self.exp_peek.begin_trial()
        elif val == "cycle": self.exp_cycle.begin_trial()
        elif val == "direct": self.exp_direct.run()
