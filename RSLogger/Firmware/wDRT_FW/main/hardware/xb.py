...
class XB:
    ...
    def _API_0x88(self, arg):
        if self._debug:
            print(f'{ticks_us()} XB._API_0x88 {arg}')
    ...
    def _API_0x91(self, msg):
        if self._debug: print(f'{ticks_us()} XB._API_0x91 {msg}')

        try:
            self._cmd = msg[21:-1].decode('utf-8')
            self._handle_incoming_message(self._cmd)
        except UnicodeError as e:
            if self._debug:
                print(f"Xbee: Unicode Error - {e}")

    def _API_0x8a(self, msg):
        if self._debug: print(f'{ticks_us()} XB._API_0x9a {msg}')
